...
# ========== Utilities ==========
def digit_function(
    f: RooFitFunction, data: RooFitData, subrange: dict = {}
) -> np.ndarray[Any]:
    """Compose function values w.r.t. function data variables.

    Args:
        f: RooFitFunction object
    ...
    data_numpy = np.array([dict_numpy[k] for k in dict_numpy.keys()])  # data array

    digit_data = np.append(
        data_numpy, np.expand_dims(digits, axis=0), axis=0
    )  # add digit function values to data array

    for k, v in subrange.items():
        if k not in dict_numpy.keys():
            raise ValueError(
                f"Only rectangular subrange is allowed! Key of subrange must be one of the function args but '{k}' is assigned"
            )
        if not isinstance(v, list):
            raise TypeError("subrange values must type of list!")
        if not len(v) == 2:
            raise ValueError(
                "subrange value list has two values: min and max of the subrange"
            )
        dict_index = list(dict_numpy.keys()).index(k)
        condition = (digit_data[:, dict_index] > v[0]) & (
            digit_data[:, dict_index] < v[1]
        )
...
if __name__ == "__main__":
    ...
    de_cb = RooFitFunction(
        ...
        {
            "x0CB": [0, -0.01, 0.01],
            "sigmacbL": [0.03, 0.001, 0.05],
            "sigmacbR": [0.01, 0.001, 0.05],
            "alphaL": [0.1, 0.005, 10],
            "nL": [1, 0.1, 20.0],
            "alphaR": [0.1, 0.005, 10],
            "nR": [1, 0.1, 20.0],
    ...
    de_bfGauss = RooFitFunction(
        ...
        {
            "x0": [0, -0.01, 0.01],
            "sigmaL": [0.02, 0.01, 0.05],
            "sigmaR": [0.02, 0.01, 0.05],
        },
    )
